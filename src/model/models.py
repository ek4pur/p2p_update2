...
def conv_block_unet(x, f, name, bn_mode, bn_axis, bn=True, strides=(2,2)):
    ...
    return x
#update I
def conv_block_unet_maxpool(x, f, name, bn_mode, bn_axis, bn=True, pool=(2,2)):
...
def up_conv_block_unet(x, x2, f, name, bn_mode, bn_axis, bn=True, dropout=False):
    ...
    return x
#update II
def up_conv_block_autoencoder(x, x2, f, name, bn_mode, bn_axis, bn=True, dropout=False):
...
#update I
def generator_unet_upsampling_maxpool(img_dim, bn_mode, model_name="generator_unet_upsampling_maxpool"):
    ...
    list_encoder = [Conv2D(list_nb_filters[0], (3, 3),
                           strides=(2, 2), name="unet_conv2D_1", padding="same")(unet_input)]
    max_p = MaxPooling2D(pool_size=(2, 2))(list_encoder[-1])
    list_encoder.append(max_p)
    for i, f in enumerate(list_nb_filters[1:]):
        name = "unet_conv2D_%s" % (i + 2)
        conv = conv_block_unet_maxpool(list_encoder[-1], f, name, bn_mode, bn_axis)
        list_encoder.append(conv)
...
#update I
def generator_unet_autoencoder(img_dim, bn_mode, model_name="generator_unet_upsampling_maxpool"):
    ...
    # Decoder
    list_decoder = [up_conv_block_autoencoder(list_encoder[-1], list_encoder[-2],
                                       list_nb_filters[0], "unet_upconv2D_1", bn_mode, bn_axis, dropout=True)]
    for i, f in enumerate(list_nb_filters[1:]):
        name = "unet_upconv2D_%s" % (i + 2)
        # Dropout only on first few layers
...
def DCGAN_discriminator(img_dim, nb_patch, bn_mode, model_name="DCGAN_discriminator", use_mbd=True):
    ...
    # 7 list : 64,128,256,512,512,512,512
    # 8 list : 64,128,256,512,512,512,512,512
...
def load(model_name, img_dim, nb_patch, bn_mode, use_mbd, batch_size):
    ...
    if model_name == "generator_unet_deconv":
        ...
        return model
    #update I
    if model_name == "generator_unet_upsampling_maxpool":
        model = generator_unet_upsampling(img_dim, bn_mode, model_name=model_name)
        model.summary()
        from keras.utils import plot_model
        plot_model(model, to_file="../../figures/%s.png" % model_name, show_shapes=True, show_layer_names=True)
    ...
    if model_name == "DCGAN_discriminator":
        ...
        return model
    #update II
    if model_name == "generator_unet_autoencoder":
        model = generator_unet_autoencoder(img_dim, bn_mode, batch_size, model_name=model_name)
        model.summary()
        from keras.utils import plot_model
        plot_model(model, to_file="../../figures/%s.png" % model_name, show_shapes=True, show_layer_names=True)
        return model
#update I
def loadDCGAN(model_name,generator_model,discriminator_model,img_dim,patch_size,image_data_format):
    ##update I
    if model_name =="DCGAN":
        model = DCGAN(generator_model,discriminator_model,img_dim,patch_size,image_data_format)
        model.summary()
        from keras.utils import plot_model
        plot_model(model, to_file="../../figures/%s.png" % model_name, show_shapes=True, show_layer_names=True)
        return model
