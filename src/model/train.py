...
def train(**kwargs):
    ...
    try:
        ...
        #                           image_data_format)
        #update I
        DCGAN_model = models.loadDCGAN("DCGAN",
                                  generator_model,
                                  discriminator_model,
                                  img_dim,
                                  patch_size,
        ...
        for e in range(nb_epoch):
            ...
            #update I
            data_utils.plot_generated_batch(X_full_batch, X_sketch_batch, generator_model,
                                            batch_size, image_data_format, "training_%s" %(e))
            X_full_batch, X_sketch_batch = next(data_utils.gen_batch(X_full_val, X_sketch_val, batch_size))
            data_utils.plot_generated_batch(X_full_batch, X_sketch_batch, generator_model,
                                            batch_size, image_data_format, "validation_%s" %(e))
        ...
        #update I
        #import pandas as pd
        #gen_path = os.path.join("../../models/gen_history.csv")
        #dis_path = os.path.join("../../models/dis_history.csv")
        #gl = pd.DataFrame(gen_loss.history)
        #gl.to_csv(gen_path)
        #dl = pd.DataFrame(disc_loss.history)
        #dl.to_csv(dis_path)
